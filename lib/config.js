...
var config = {
  ...
  title: process.env.FORIM_TITLE || 'node全栈',
  mongoose: {
  ...
  name: process.env.FORIM_NAME || 'Node开源全栈开者交流社区', // 社区名字
  ...
  google_tracker_id: process.env.FORIM_GOOGLE_TRACKER_ID || '',
  ...
  ip: process.env.OPENSHIFT_NODEJS_IP || process.env.FORIM_HOST || '127.0.0.1',
  ...
  // RSS配置
  rss: {
    title: process.env.FORIM_RSS_TITLE || 'Node全栈社区',
  ...
  // admin 可删除话题，编辑标签。
  // 把登录名放入数组即可
  admins: [process.env.FORIM_ADMIN],
  ...
  GITHUB_OAUTH: {
    ...
    callbackURL: process.env.FORIM_GITHUB_CALLBACK_URL || 'http://' + siteName + '/auth/github/callback'
  },
