...
var config = {
  im: {
    site: process.env.FORIM_IM_SITE || 'http://xiv.im'
  },
  cors: {
    allowed: process.env.FORIM_CORS_DOMAIN || 'xiv.im:* im.t1bao.com:* local.xiv.im:*'
  },
  limits: {
    normal: 20,
    short: 5
  ...
  description: process.env.FORIM_DESCRIPTION || '全栈开者交流社区', // 社区的描述
  keywords: process.env.FORIM_KEYWORDS || '全栈开发者',
  ...
  // 程序运行的端口
  port: process.env.OPENSHIFT_NODEJS_PORT || process.env.NODE_PORT || process.env.FORIM_PORT || 3000,
  // 话题列表显示的话题数量
  ...
  rss: {
    ...
    description: process.env.FORIM_RSS_DESCRIPTION || '全栈开发社区',
    // 最多获取的RSS Item数量
