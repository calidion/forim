...
exports.login = function (req, res, next) {
  ...
  getUser(loginname, function (err, user) {
    ...
    tools.bcompare(pass, passhash, ep.done(function (bool) {
      ...
      if (!user.active) {
        // 重新发送激活邮件
        return mailer.user.activate(user.email, utility.md5(user.email + passhash + config.session_secret), user.loginname,
          function () {
            res.status(403);
            return res.render('sign/signin', {
...
exports.updateSearchPass = function (req, res, next) {
  ...
  User.getUserByMail(email, function (err, user) {
    ...
    user.save(function (err) {
      ...
      // 发送重置密码邮件
      mailer.user.password.reset(email, retrieveKey, user.loginname, function () {
        res.render('notify/notify', {
