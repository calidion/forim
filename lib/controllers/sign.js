...
var mail2 = require('../v2/util/mailer');
  ...
  User.getUsersByQuery({
    ...
    tools.bhash(pass, ep.done(function (passhash) {
      ...
      User.newAndSave(loginname, loginname, passhash, email, avatarUrl, false, function (err) {
        ...
        // 发送激活邮件
        mail2.user.activate(email, utility.md5(email + passhash + config.session_secret), loginname, function () {
          res.render('sign/signup', {
...
exports.login = function (req, res, next) {
  ...
  getUser(loginname, function (err, user) {
    ...
    tools.bcompare(pass, passhash, ep.done(function (bool) {
      ...
      if (!user.active) {
        // 重新发送激活邮件
        return mail2.user.activate(user.email, utility.md5(user.email + passhash + config.session_secret), user.loginname,
          function () {
            res.status(403);
            return res.render('sign/signin', {
...
exports.updateSearchPass = function (req, res, next) {
  ...
  User.getUserByMail(email, function (err, user) {
    ...
    user.save(function (err) {
      ...
      // 发送重置密码邮件
      mail2.user.password.reset(email, retrieveKey, user.loginname, function () {
        res.render('notify/notify', {
